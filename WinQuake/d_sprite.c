...
void D_SpriteCalculateGradients (void)
{
	...
	TransformVector_T (r_spritedesc.vpn, p_normal);
	TransformVector_T (r_spritedesc.vright, p_saxis);
	TransformVector_T (r_spritedesc.vup, p_taxis);
	...
	TransformVector_T (modelorg, p_temp1);
...
// wrapping
	...
	D_SpriteScanLeftEdge ();
	D_SpriteScanRightEdge ();
	D_SpriteDrawSpans_T (sprite_spans);
}
